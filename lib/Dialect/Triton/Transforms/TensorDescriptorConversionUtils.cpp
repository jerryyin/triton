...
// Filter out operand segment sizes from the list of attributes since
...
Value getExpandedOffsetWithRange(OpBuilder &builder, Location loc,
                                 ArrayRef<int64_t> blockShape, Value offset,
                                 unsigned dim) {
  // Add range
...
Value generatePtr(OpBuilder &builder, Location loc,
...
Value generateMaskFromOffsetRanges(OpBuilder &builder, Location loc,
                                   ArrayRef<int64_t> blockShape,
...
Value generateMask(OpBuilder &builder, Location loc,
                   ArrayRef<int64_t> blockShape, Descriptor &desc,
                   ValueRange offsets) {
  assert(blockShape.size() == desc.shape.size());
  assert(blockShape.size() == offsets.size());
...
Value generateOther(OpBuilder &builder, Location loc, Type scalarTy,
  ...
  if (paddingOption && isa<FloatType>(scalarTy)) {
    auto floatTy = cast<FloatType>(scalarTy);
    ...
    return arith::SelectOp::create(builder, loc, paddingOption, nanValue,
                                   zeroValue);
  } else {
...
SmallVector<Value> castToI64(OpBuilder &builder, ValueRange values) {
...
struct RewriteMakeTensorDesc : OpConversionPattern<triton::MakeTensorDescOp> {
  ...
  matchAndRewrite(triton::MakeTensorDescOp op, OpAdaptor adaptor,
    ...
    SmallVector<Value> ptrShapeStridesPaddingOption;
    ...
    auto paddingOption = arith::ConstantOp::create(
...
LogicalResult convertTensorDescTypeToPointer(triton::TensorDescType t,
  ...
  out.insert(out.end(), 2 * tensorType.getRank(),
             IntegerType::get(t.getContext(), 64));
  out.push_back(IntegerType::get(t.getContext(), 1));
  out.push_back(IntegerType::get(t.getContext(), 1));
  return success();
}

TypeConverter createDescTypeConverter() {
  ...
  converter.addConversion(convertTensorDescTypeToPointer);
...
void populateMakeTensorDescriptorPattern(RewritePatternSet &patterns,
                                         TypeConverter &converter) {
  patterns.add<RewriteMakeTensorDesc>(converter, patterns.getContext());
}

void populateLoadTensorDescriptorPattern(RewritePatternSet &patterns,
                                         TypeConverter &converter) {
  patterns.add<RewriteLoadPattern>(converter, patterns.getContext());
}

void populateStoreTensorDescriptorPattern(RewritePatternSet &patterns,
                                          TypeConverter &converter) {
  patterns.add<RewriteStorePattern>(converter, patterns.getContext());
}

void populateGatherDescriptorPattern(RewritePatternSet &patterns,
                                     TypeConverter &converter) {
  patterns.add<RewriteGatherPattern>(converter, patterns.getContext());
}

void populateScatterTensorDescriptorPattern(RewritePatternSet &patterns,
                                            TypeConverter &converter) {
  patterns.add<RewriteScatterPattern>(converter, patterns.getContext());
}

void populateReduceTensorDescriptorPattern(RewritePatternSet &patterns,
                                           TypeConverter &converter) {
  patterns.add<RewriteReducePattern>(converter, patterns.getContext());
}
} // namespace mlir::triton
